...
class TestWebhookEventClaim:
    @pytest.mark.asyncio
    async def test_second_claim_is_rejected(self):
        from api.routes.orders import claim_razorpay_event

        client = AsyncMock()
        client.set = AsyncMock(side_effect=[True, None])
        with patch(
            "api.routes.orders.redis_client.get_client",
            new_callable=AsyncMock,
            return_value=client,
        ):
            first = await claim_razorpay_event("payment.captured", "pay_abc")
            second = await claim_razorpay_event("payment.captured", "pay_abc")
