...
MAIN_KEY = "main"
CACHE_KEY = "chokmoki:account_page"
CACHE_TTL = 600


class AccountPageSettingsService:
    COLLECTION_NAME = "account_page_settings"
    ...
    async def get_public(self) -> Optional[Dict[str, Any]]:
        ...
        result = AccountPageSettings(**doc).model_dump(by_alias=True)
        await cache.set(CACHE_KEY, mongo_json_dumps(result), CACHE_TTL)
        return result

    async def get_admin(self) -> Optional[AccountPageSettings]:
        database = await db.get_database()
        doc = await database[self.COLLECTION_NAME].find_one({"settings_key": MAIN_KEY})
        if doc:
            return AccountPageSettings(**doc)
        return None

    async def upsert(self, data: AccountPageSettingsUpdate) -> AccountPageSettings:
        database = await db.get_database()
        ...
        logger.info("Account page settings upserted")
        return AccountPageSettings(**doc)
