...
from src.models.navigation_settings import NavigationSettings, NavigationSettingsUpdate
...
class NavigationSettingsService:
    COLLECTION_NAME = "navigation_settings"
    ...
    async def get_public(self) -> Optional[Dict[str, Any]]:
        ...
        result = NavigationSettings(**doc).model_dump(by_alias=True)
        await cache.set(CACHE_KEY, mongo_json_dumps(result), CACHE_TTL)
        return result

    async def get_admin(self) -> Optional[NavigationSettings]:
        database = await db.get_database()
        doc = await database[self.COLLECTION_NAME].find_one({"settings_key": MAIN_KEY})
        if doc:
            return NavigationSettings(**doc)
        return None

    async def upsert(self, data: NavigationSettingsUpdate) -> NavigationSettings:
        database = await db.get_database()
        ...
        logger.info("Navigation settings upserted")
        return NavigationSettings(**doc)
