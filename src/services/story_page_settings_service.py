...
from src.models.story_page_settings import StoryPageSettings, StoryPageSettingsUpdate
from src.services.cache_service import cache
from src.utils.mongo_json import mongo_json_dumps
from src.plugins.logger import logger

MAIN_KEY = "main"
CACHE_KEY = "chokmoki:story_page"
CACHE_TTL = 600


class StoryPageSettingsService:
    COLLECTION_NAME = "story_page_settings"
    ...
    async def get_public(self) -> Optional[Dict[str, Any]]:
        ...
        result = StoryPageSettings(**doc).model_dump(by_alias=True)
        await cache.set(CACHE_KEY, mongo_json_dumps(result), CACHE_TTL)
        return result

    async def get_admin(self) -> Optional[StoryPageSettings]:
        database = await db.get_database()
        doc = await database[self.COLLECTION_NAME].find_one({"settings_key": MAIN_KEY})
        if doc:
            return StoryPageSettings(**doc)
        return None

    async def upsert(self, data: StoryPageSettingsUpdate) -> StoryPageSettings:
        database = await db.get_database()
        ...
        logger.info("Story page settings upserted")
        return StoryPageSettings(**doc)
